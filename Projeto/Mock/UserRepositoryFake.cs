...
namespace Projeto.Mock
{
    public class UserRepositoryFake
    {
        ...
        public UserRepositoryFake()
        {
            ...
            // admin
            var user = new User() { Id = 1, Username = "admin", Password = "1234", Role = "Admin", Active = true };
            List.Add(user);

            // Usuario comum ativo:
            user = new User() { Id = 2, Username = "usuario", Password = "1234", Role = "User", Active = true };
            List.Add(user);

            // Usuario comum inativo:
            user = new User() { Id = 3, Username = "inativo", Password = "1234", Role = "User", Active = false };
            List.Add(user);

            // Usuario cadastrado, aguardando liberação de acesso (feito pelo Admin):
            user = new User() { Id = 4, Username = "cadastrado", Password = "1234", Role = null, Active = false };
            List.Add(user);
        }
        ...
        public User ActivateFirstAccess(int id, string role)
        {
        ...
        public bool ExistsUpdate(User model, int id)
        {
